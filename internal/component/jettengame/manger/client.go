...
import (
	...
	lconf "sanguoxiao/internal/component/jettengame/conf"
...
// 添加玩家
func (self *ClientManger) Append(userID int64, agent gate.Agent) bool {
...
// 客户端连接是否存在
func (self *ClientManger) Get(userID int64) (gate.Agent, bool) {
	if v, ok := self.Load(userID); ok {
		agent := v.(gate.Agent)
...
func (self *ClientManger) NotifyAll(msg proto.Message) {
	self.Range(func(key, value interface{}) bool {
		...
		if nil != userData {
			...
			//	return true
			//}
			log.Debug("通知玩家：%v %v", user, user.Account)
		} else {
...
func (self *ClientManger) Marshal(m proto.Message) ([]byte, error) {
	data, err := msg.ProcessorProto.Marshal(m)
	if err != nil {
		return nil, err
	}
...
func (self *ClientManger) Unmarshal(data []byte) (interface{}, error) {
	return msg.ProcessorProto.Unmarshal(data)
