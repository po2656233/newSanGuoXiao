...
func getTilesScore(s []int) int {
	...
	var pos = findDuiPos(tmpCard)
	score7Dui = len(pos) * 2
	log.Release("[mahjong LIB]牌型7对分:%v", score7Dui)

	for _, v := range pos {
		var tmp = removeDui(tmpCard, v)
		score, remainTiles := getTilesScoreWithoutDui(tmp)
		log.Release("[mahjong LIB]牌型带对积分:%v,对:%v,剩:%v", score+2, tmpCard[v], remainTiles)
	...
	log.Release("[mahjong LIB]牌型不带对积分:%v,剩:%v", scoreWithoutDui, remainTiles)
...
func findKeOrShunCnt(s []int) (int, []int) {
	...
	for {
		...
		sort.Ints(s)
		if firstIsKe(s) {
			// log.Release("[mahjong LIB]找到刻:%v", s[:3])
			s = s[3:]
			cnt++
		} else if firstIsShun(s) {
			// log.Release("[mahjong LIB]找到顺:%v,%v,%v", s[0], s[0]+1, s[0]+2)
	...
	// log.Release("[mahjong LIB]findKeOrShunCnt:%v", cnt)
