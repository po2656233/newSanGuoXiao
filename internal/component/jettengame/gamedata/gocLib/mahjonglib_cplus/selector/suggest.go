...
import (
	...
	"sanguoxiao/internal/component/jettengame/gamedata/gocLib/util"
)
...
func (ms *MSelector) GetSuggest() int {
	...
	if tingMap := getTingMap(s); len(tingMap) > 0 {
		log.Release("[mahjong LIB]此牌型已听牌")
		selectTile := 0
		selectTileTingCnt := -1

		for tile, tingTiles := range tingMap {
			tingCnt := ms.getRemainTilesCnt(tingTiles)
			log.Release("[mahjong LIB]打出:%v, 可听:%v, 剩余(张):%v", tile, tingTiles, tingCnt)
...
func (ms *MSelector) suggestByAIGold(s []int) int {
	log.Release("[mahjong LIB]-------------------------------荣耀黄金--------------------------------")

	// 计算当前手牌所处的阶段
	unPlayStep := getTilesStep(s)
	log.Release("[mahjong LIB]当前手牌牌阶:%v", unPlayStep)
	...
	for _, playTile := range util.SliceUniqueInt(s) {
		...
		if step > unPlayStep {
			sort.Ints(effects)
			log.Release("[mahjong LIB]打出:%v,手牌:%v, 一类有效牌:%v(%v)(remain:%v)------------", playTile, tiles, effects, len(effects), ms.getRemainTilesCnt(effects))
	...
	if len(maxEffectTiles) > 1 {
		log.Release("[mahjong LIB]存在多张有效牌相同的打法，根据权重重新筛选一次:%v", maxEffectTiles)
...
func (ms *MSelector) suggestByAIPlatinum(s []int) int {
	...
	if unPlayStep < 3 {
		log.Release("[mahjong LIB]当前牌阶低于3级, 直接按照权重选牌")
		tilesWeight := getTilesWeight(s, nil)
		return ms.selectByWeightAndRemain(tilesWeight)
	}
	...
	// 如果存在相同的一类有效牌，则根据权重再取一次
	log.Release("[mahjong LIB]maxEffectTiles:%v", maxEffectTiles)
	if len(maxEffectTiles) > 1 {
		log.Release("[mahjong LIB]存在多张有效牌相同的打法，根据剩余牌权重重新筛选一次:%v", maxEffectTotalWeights)

		// 读取权重最大的牌
		maxWeightTiles, _ := getMaxValueSlice(maxEffectTotalWeights)
		log.Release("[mahjong LIB]权重最大的牌:%v", maxWeightTiles)
...
func (ms *MSelector) suggestByAIDiamond(s []int) int {
	log.Release("[mahjong LIB]-------------------------------璀璨钻石--------------------------------")

	// // 未打出时的阶段
	// unPlayStep := getTilesStep(s)
	// log.Release("[mahjong LIB]未打牌的阶段:%v", unPlayStep)
	...
	// 		// log.Release("[mahjong LIB]可能是二类的有效牌:%v", relationTiles)
	// 		secondEffects := []int{}
	...
	// 						log.Release("[mahjong LIB]%v替换成%v, 一类有效牌变为:%v(%v)(remain:%v)", replacedTile, tile, curEffects, len(curEffects), ms.getRemainTilesCnt(curEffects))
	...
	// 		log.Release("[mahjong LIB]二类有效牌:%v(%v)(remain:%v)", secondEffects, len(secondEffects), ms.getRemainTilesCnt(secondEffects))
	// 	}
	// 	// break
	// }

	// log.Release("[mahjong LIB]总计算次数:%v, 耗时:%v", times, (time.Now().UnixNano() - start))
...
func (ms *MSelector) selectByWeightAndRemain(tilesWeight map[int]int) int {
	...
	log.Release("[mahjong LIB]权重最小的牌:%v", minWeightTiles)
