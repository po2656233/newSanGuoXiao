...
//func (sqlSelf *SqlOrm) CheckGameRecordLast(gid, uid int64) (sqlmodel.GameRecord, error) {
//	record := sqlmodel.GameRecord{}
...
func (sqlSelf *SqlOrm) CheckAllGoods() (goodsInfoList map[int64]*protoMsg.GoodsInfo) {
	goodsList := make([]*sqlmodel.Goods, 0)
	goods := sqlmodel.Goods{}
	fields := "*"
...
func (sqlSelf *SqlOrm) CheckAssets(userID int64) *protoMsg.KnapsackInfo {
	assetList := make([]sqlmodel.Assetgoods, 0)
...
//func (sqlSelf *SqlOrm) CheckRoomInfo(roomNum int64) *sqlmodel.Room {
//	room := sqlmodel.Room{}
...
//		gameList := make([]sqlmodel.Game, 0)
//		fields = "`id`,`name`,`kindid`,`level`,`hostid`,`robot_count`"
//		err := sqlSelf.db.Table((&sqlmodel.Game{}).TableName()).Select(fields).Where("type=?", room.Type).Find(&gameList).Error
//		if !CheckError(err) {
//			return "", "", nil
//		}
...
func (sqlSelf *SqlOrm) CheckInnings(uid, gid int64) []*protoMsg.InningInfo {
	recordList := make([]*sqlmodel.Record, 0)
...
func (sqlSelf *SqlOrm) CheckRecharges(uid int64) []*protoMsg.RechargeResp {
	rechargeList := make([]*sqlmodel.Recharge, 0)
...
func (sqlSelf *SqlOrm) CheckGetCheckIn(userID int64) []*protoMsg.CheckInResp {
	checkinList := make([]*sqlmodel.Checkin, 0)
	checkin := sqlmodel.Checkin{}
...
func (sqlSelf *SqlOrm) CheckEmails(userID int64) []*protoMsg.EmailInfo {
	...
	emailList := make([]*sqlmodel.Email, 0)
	emailM := sqlmodel.Email{}
...
func (sqlSelf *SqlOrm) CheckNotice(platID int64) []*protoMsg.NotifyNoticeResp {
	noticeList := make([]*sqlmodel.Notice, 0)
	noticeM := sqlmodel.Notice{}
	...
	for _, m := range noticeList {
		notice := &protoMsg.NotifyNoticeResp{
			UserID:  constant.INVALID,
			GameID:  constant.INVALID,
...
// AddUser 完成注册 【新增一个玩家】
func (sqlSelf *SqlOrm) AddUser(user sqlmodel.User) (int64, error) {
	sqlSelf.Lock()
	defer sqlSelf.Unlock()
...
func (sqlSelf *SqlOrm) AddEmail(userID, toID int64, carboncopy, topic, content, goods, remark string) (int64, error) {
	sqlSelf.Lock()
	defer sqlSelf.Unlock()
	sender := hints.StatusText[hints.Title008]
...
// AddNotice 公告
func (sqlSelf *SqlOrm) AddNotice(notice sqlmodel.Notice) (int64, error) {
	sqlSelf.Lock()
	defer sqlSelf.Unlock()
...
//	}
//
//	if constant.INVALID < checkin.Timestamp {
...
//	if person == nil {
//		clog.Error("无效的玩家数据%v  时间:%v", userID, time.Now().Unix())
...
//	sqlSelf.Lock()
//	defer sqlSelf.Unlock()
//	assetList := make([]*sqlmodel.Asset, 0)
//	asset := sqlmodel.Asset{}
