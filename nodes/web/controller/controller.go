...
func (p *Controller) classList(c *superGin.Context) {
	data, statusCode := rpc.SendData(p.App, rpc.SourcePath, rpc.HomeActor, rpc.CenterType, &pb.GetClassListReq{})
	if statusCode != code.OK {
		RenderResult(c, http.StatusBadRequest)
		return
	}
	//获取分类列表
	resp, ok := data.(*pb.GetClassListResp)
	if !ok {
		RenderResult(c, http.StatusBadRequest, map[string]string{
			"data": "bad data!",
		})
		return
	}
...
func (p *Controller) roomList(c *superGin.Context) {
	cid := c.GetInt32("cid", 0)
	if cid < 1 {
		sgxLogger.Warnf("cid < 1 params=%v", c.GetParams())
		RenderResult(c, Service003, StatusText[Service003])
		return
	}
	data, statusCode := rpc.SendData(p.App, rpc.SourcePath, rpc.HomeActor, rpc.CenterType, &pb.ChooseClassReq{
		Id: cid,
	})
	if statusCode != code.OK {
		RenderResult(c, http.StatusOK, map[string]string{
	...
	//获取房间列表
	resp, ok := data.(*pb.ChooseClassResp)
	if !ok {
		RenderResult(c, http.StatusNoContent, map[string]string{
			"data": "bad data!",
		})
		return
	}
...
func (p *Controller) roomCreate(c *superGin.Context) {
	hostid, ok := c.GetPostForm("hostid")
	if !ok {
...
func (p *Controller) login(c *superGin.Context) {
	...
	sdkInvoke.Login(config, params, func(statusCode int32, result sdk.Params, error ...error) {
		...
		strToken, tkErr := token.CreateToken(params[Username], timeout)
		if tkErr != nil {
