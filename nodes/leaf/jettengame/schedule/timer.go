...
func (tm *Timer) Cancel(item *TimerItem) {
	...
	n := tm.timers.Len()
	if n == 0 {
		log.Printf("Timer(%v) Cancel Error: Timer Size Is 0!", tm.tag)
		return
	}
	if item.Index > 0 && item.Index < n {
		if item != tm.timers[item.Index] {
			log.Printf("Timer(%v) Cancel Error: Invalid Item!", tm.tag)
			return
		}
		...
		log.Printf("Timer(%v) Cancel Error: Invalid Index: %d", tm.tag, item.Index)
	}
}
...
func (tm *Timer) Reset(item *TimerItem, delay time.Duration) {
	...
	n := tm.timers.Len()
	if n == 0 {
		log.Printf("Timer(%s) Reset Error: Timer Size Is 0!", tm.tag)
		return
	}
	if item.Index < n {
		if item != tm.timers[item.Index] {
			log.Printf("Timer(%s) Reset Error: Invalid Item!", tm.tag)
			return
		}
