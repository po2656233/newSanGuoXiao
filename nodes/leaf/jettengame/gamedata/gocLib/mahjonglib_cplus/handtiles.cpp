...
extern "C"
{
    ...
    namespace mahjong
    {
        ...
        int Handtiles::HuapaiCount() const
        {
            ...
            {
                std::unordered_map<int, int>::const_iterator iter = huapai_table.find(i);
                if (iter != huapai_table.end())
                {
                    cnt += iter->second;
                }
            }
        ...
        int Handtiles::StringToHandtiles(const std::string &s_ori)
        {
            ...
            for (auto c : s)
            {
                ...
                case ']':
                {
                    ...
                    { //处理三张的副露
                        ...
                        if (!is_chars && tiles[1] == tiles[0] + 1 && tiles[1] == tiles[2] - 1)
                        { //吃
            ...
            { //杠状态下，
                if (IsZimo())
                { //若为自摸，则为杠上开花：手里必须有杠（更严格的话是最后一个副露必须为杠）
                ...
                { //若为铳和，则为抢杠和：非海底状态（能加杠说明不是海底），所和的牌必须手里没有
        ...
        int Handtiles::_GenerateTable()
        {
            ...
            if (TILE_DONG - TILE_MEI <= huapai_table.size())
            {
                for (int i = TILE_MEI; i <= TILE_DONG; i++)
                {
